...
import { createInboundOrder } from "../../services/inboundOrderApi";
...
import {
  validateInboundOrderCreateForm,
  INBOUND_ORDER_CODE_MAX,
  INBOUND_ORDER_SOURCE_CODE_MAX,
  INBOUND_ORDER_SOURCE_DEPT_MAX,
  INBOUND_ORDER_DEST_WAREHOUSE_MAX,
  type InboundOrderFieldErrors,
} from "../../utils/inboundOrderFormValidation";

export type InboundOrderLineRow = OutboundLineInput;
...
function newLine(): InboundOrderLineRow {
  return {
...
function lineSubtotal(row: InboundOrderLineRow): number {
...
/** Tạo mới lệnh nhập kho — lưu qua API `/api/inbound-orders`. */
export function InboundOrderCreateScreen({ onBack, userDisplayName = "Người dùng" }: Props): ReactElement {
  const [excelImportOpen, setExcelImportOpen] = useState(false);
  const [productPickerOpen, setProductPickerOpen] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const [lines, setLines] = useState<InboundOrderLineRow[]>(() => [newLine()]);
  ...
  const [fieldErrors, setFieldErrors] = useState<InboundOrderFieldErrors>({});
  ...
  const updateLine = (key: string, patch: Partial<InboundOrderLineRow>): void => {
    ...
    <div className="app-page outbound-page out-create-page feature-doc-page">
      ...
      <main className="outbound-main out-create-main">
        ...
        <p className="out-create-crumb muted">
          Xuất - nhập với Nội bộ <span className="out-create-crumb-sep">›</span> Tạo mới lệnh nhập kho
        </p>
        <h1 className="out-create-doc-title">Lệnh nhập kho</h1>
        ...
        <section className="card out-create-section">
          ...
          <div className="form-grid">
            <label className="field">
              ...
              <input
                value={sourceDept}
                maxLength={INBOUND_ORDER_SOURCE_DEPT_MAX}
                aria-invalid={Boolean(fieldErrors.source_dept)}
                onChange={(e) => {
                  setSourceDept(e.target.value);
                  setFieldErrors((p) => {
                    const n = { ...p };
                    delete n.source_dept;
                    return n;
                  });
                }}
                placeholder="VD: Chi nhánh Hà Nội"
              />
              {fieldErrors.source_dept ? <p className="inline-error">{fieldErrors.source_dept}</p> : null}
            </label>
            <label className="field">
              <span>Mã nguồn</span>
              <input
                value={sourceCode}
                maxLength={INBOUND_ORDER_SOURCE_CODE_MAX}
            ...
            <label className="field">
              ...
              <input
                value={destWarehouse}
                maxLength={INBOUND_ORDER_DEST_WAREHOUSE_MAX}
                aria-invalid={Boolean(fieldErrors.dest_warehouse)}
                onChange={(e) => {
                  setDestWarehouse(e.target.value);
                  setFieldErrors((p) => {
                    const n = { ...p };
                    delete n.dest_warehouse;
                    return n;
                  });
                }}
                placeholder="VD: Kho tổng"
              />
              {fieldErrors.dest_warehouse ? <p className="inline-error">{fieldErrors.dest_warehouse}</p> : null}
            </label>
            <label className="field">
              <span>Ngày yêu cầu nhập</span>
              <input
                type="date"
                value={needByDate}
            ...
            <label className="field">
              ...
              <input
                value={orderCode}
                maxLength={INBOUND_ORDER_CODE_MAX}
        ...
        <div className="out-create-actions">
          ...
          <button
            ...
            onClick={() => {
              void (async () => {
                setSubmitError(null);
                const checked = validateInboundOrderCreateForm({
                  orderCode,
                  sourceDept,
                  sourceCode,
                  destWarehouse,
                ...
                try {
                  await createInboundOrder({
                    ...
                    sourceDept: sourceDept.trim(),
                    sourceCode: sourceCode.trim(),
                    destWarehouse: destWarehouse.trim(),
                  ...
                  setSubmitError(getHttpErrorMessage(err, "Không lưu được lệnh nhập."));
                } finally {
                  setSaving(false);
                }
              })();
